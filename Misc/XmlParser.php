...
class XmlParser
{
        ...
        if (!xml_parse($this->xml_parser, $data))
            ...
            if ($error_code == 4)
                ...
                if (!xml_parse($this->xml_parser, $data))
                    ...
                    $this->content[0]['title'] = "XML ошибка в файле: " . $file;
                    $this->content[0]['description'] = sprintf("XML ошибка: %s в строке %d", xml_error_string(xml_get_error_code($this->xml_parser)), xml_get_current_line_number($this->xml_parser));
                    $this->content[0]['link'] = "#";
                    $this->content[0]['date'] = time();
                    }
                } else
                ...
                $this->content[0]['title'] = "XML ошибка в файле: " . $file;
                $this->content[0]['description'] = sprintf("XML ошибка: %s в строке %d", xml_error_string($error_code), $error_line);
                $this->content[0]['link'] = "#";
                $this->content[0]['date'] = time();
                }
            }
    ...
    function _get_contents($file)
    {
        ...
        if (function_exists('curl_init'))
            ...
            if ($data)
                ...
                return false;
            } else  // если его нет, то получаем файл через file_get_contents
            {
    ...
    function get_charset($charset)
    {
        ...
        if ($this->rss_charset == '')
            $this->rss_charset = strtolower($charset);
    }
    ...
    function convert($from, $to)
    {
        ...
        if (function_exists('iconv'))
            ...
            foreach ($this->content as $content)
            {
                ...
                if ($this->content[$i]['category'] and @iconv($from, $to . "//IGNORE", $this->content[$i]['category']))
                    $this->content[$i]['category'] = @iconv($from, $to . "//IGNORE", $this->content[$i]['category']);

                if ($this->content[$i]['author'] and @iconv($from, $to . "//IGNORE", $this->content[$i]['author']))
                    $this->content[$i]['author'] = @iconv($from, $to . "//IGNORE", $this->content[$i]['author']);
